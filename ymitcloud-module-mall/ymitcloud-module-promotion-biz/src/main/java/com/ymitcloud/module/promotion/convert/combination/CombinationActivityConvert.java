package com.ymitcloud.module.promotion.convert.combination;
...
public interface CombinationActivityConvert {
    ...
    default PageResult<CombinationActivityPageItemRespVO> convertPage(PageResult<CombinationActivityDO> page,
            List<CombinationProductDO> productList, Map<Long, Integer> groupCountMap,
            Map<Long, Integer> groupSuccessCountMap, Map<Long, Integer> recordCountMap,
    ...
    @Mappings({ @Mapping(target = "id", ignore = true), @Mapping(target = "activityId", source = "activity.id"),
            @Mapping(target = "spuId", source = "activity.spuId"), @Mapping(target = "skuId", source = "product.skuId"),
    ...
    default List<CombinationProductDO> convertList(List<? extends CombinationProductBaseVO> products,
            CombinationActivityDO activity) {
        return CollectionUtils.convertList(products,
                item -> convert(activity, item).setActivityStatus(activity.getStatus()));
    }
    ...
    default List<CombinationProductDO> convertList(List<CombinationProductBaseVO> updateProductVOs,
        ...
        Map<Long, Long> productMap = CollectionUtils.convertMap(products, CombinationProductDO::getSkuId,
                CombinationProductDO::getId);
        return CollectionUtils.convertList(updateProductVOs, updateProductVO -> convert(activity, updateProductVO)
                .setId(productMap.get(updateProductVO.getSkuId())).setActivityStatus(activity.getStatus()));
    ...
    default CombinationRecordDO convert(CombinationRecordCreateReqDTO reqDTO, CombinationActivityDO activity,
            MemberUserRespDTO user, ProductSpuRespDTO spu, ProductSkuRespDTO sku) {
        return convert(reqDTO).setVirtualGroup(false).setStatus(CombinationRecordStatusEnum.IN_PROGRESS.getStatus()) // 创建后默认状态为进行中
    ...
    default List<AppCombinationActivityRespVO> convertAppList(List<CombinationActivityDO> list,
            ...
            List<CombinationProductDO> productList, List<ProductSpuRespDTO> spuList) {
        List<AppCombinationActivityRespVO> activityList = convertAppList(list);
        Map<Long, ProductSpuRespDTO> spuMap = CollectionUtils.convertMap(spuList, ProductSpuRespDTO::getId);
        Map<Long, List<CombinationProductDO>> productMap = CollectionUtils.convertMultiMap(productList,
                CombinationProductDO::getActivityId);
        return CollectionUtils.convertList(activityList, item -> {
            // 设置 product 信息
            item.setCombinationPrice(CollectionUtils.getMinValue(productMap.get(item.getId()),
                    CombinationProductDO::getCombinationPrice));
            // 设置 SPU 信息
            findAndThen(spuMap, item.getSpuId(),
                    spu -> item.setPicUrl(spu.getPicUrl()).setMarketPrice(spu.getMarketPrice()));
    ...
    default AppCombinationActivityDetailRespVO convert3(CombinationActivityDO combinationActivity,
    ...
    default PageResult<CombinationRecordPageItemRespVO> convert(PageResult<CombinationRecordDO> recordPage,
            List<CombinationActivityDO> activities, List<CombinationProductDO> products) {
        PageResult<CombinationRecordPageItemRespVO> result = convert(recordPage);
        // 拼接关联属性
        Map<Long, CombinationActivityDO> activityMap = CollectionUtils.convertMap(activities,
                CombinationActivityDO::getId);
        Map<Long, List<CombinationProductDO>> productsMap = CollectionUtils.convertMultiMap(products,
                CombinationProductDO::getActivityId);
    ...
    default AppCombinationRecordDetailRespVO convert(Long userId, CombinationRecordDO headRecord,
