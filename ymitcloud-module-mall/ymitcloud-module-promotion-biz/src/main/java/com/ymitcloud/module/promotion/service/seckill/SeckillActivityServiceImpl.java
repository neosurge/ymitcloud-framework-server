package com.ymitcloud.module.promotion.service.seckill;
...
import com.ymitcloud.module.promotion.api.seckill.dto.SeckillValidateJoinRespDTO;
...
import com.ymitcloud.module.promotion.controller.app.seckill.vo.activity.AppSeckillActivityPageReqVO;
...
import com.ymitcloud.module.promotion.dal.mysql.seckill.seckillactivity.SeckillActivityMapper;
import com.ymitcloud.module.promotion.dal.mysql.seckill.seckillactivity.SeckillProductMapper;
...
import static com.ymitcloud.framework.common.util.date.LocalDateTimeUtils.isBetween;
import static com.ymitcloud.module.product.enums.ErrorCodeConstants.SKU_NOT_EXISTS;
...
public class SeckillActivityServiceImpl implements SeckillActivityService {
    ...
    public Long createSeckillActivity(SeckillActivityCreateReqVO createReqVO) {
        ...
        SeckillActivityDO activity = SeckillActivityConvert.INSTANCE.convert(createReqVO)
                ...
                .setStatus(CommonStatusEnum.ENABLE.getStatus()).setStock(CollectionUtils
                        .getSumValue(createReqVO.getProducts(), SeckillProductBaseVO::getStock, Integer::sum));
        activity.setTotalStock(activity.getStock());
        seckillActivityMapper.insert(activity);
        // 2.2 插入商品
        List<SeckillProductDO> products = SeckillActivityConvert.INSTANCE.convertList(createReqVO.getProducts(),
                activity);
    ...
    private void validateProductConflict(List<Long> configIds, Long spuId, Long activityId) {
        ...
        List<SeckillActivityDO> activityList = seckillActivityMapper
                .selectListByStatus(CommonStatusEnum.ENABLE.getStatus());
        ...
        List<SeckillActivityDO> conflictActivityList = CollectionUtils.filterList(activityList,
                s -> CollectionUtils.containsAny(s.getConfigIds(), configIds));
    ...
    private void validateProductExists(Long spuId, List<SeckillProductBaseVO> products) {
        ...
        Map<Long, ProductSkuRespDTO> skuMap = CollectionUtils.convertMap(skus, ProductSkuRespDTO::getId);
    ...
    public void updateSeckillActivity(SeckillActivityUpdateReqVO updateReqVO) {
        ...
        SeckillActivityDO updateObj = SeckillActivityConvert.INSTANCE.convert(updateReqVO).setStock(
                CollectionUtils.getSumValue(updateReqVO.getProducts(), SeckillProductBaseVO::getStock, Integer::sum));
    ...
    private void updateSeckillProduct(SeckillActivityDO activity, List<SeckillProductBaseVO> products) {
        ...
        List<List<SeckillProductDO>> diffList = CollectionUtils.diffList(oldList, newList, (oldVal, newVal) -> {
        ...
        if (isNotEmpty(diffList.get(2))) {
            ...
            seckillProductMapper.deleteBatchIds(CollectionUtils.convertList(diffList.get(2), SeckillProductDO::getId));
    ...
    public void deleteSeckillActivity(Long id) {
        ...
        if (CommonStatusEnum.ENABLE.getStatus().equals(seckillActivity.getStatus())) {
            ...
            throw ServiceExceptionUtil
                    .exception(ErrorCodeConstants.SECKILL_ACTIVITY_DELETE_FAIL_STATUS_NOT_CLOSED_OR_END);
        ...
        seckillProductMapper.deleteBatchIds(CollectionUtils.convertSet(products, SeckillProductDO::getId));
    ...
    public List<SeckillActivityDO> getSeckillActivityListByConfigIdAndStatus(Long configId, Integer status) {
        ...
        return CollectionUtils.filterList(seckillActivityMapper.selectList(SeckillActivityDO::getStatus, status),
                item -> CollectionUtils.anyMatch(item.getConfigIds(), id -> ObjectUtil.equal(id, configId)) // 校验时段
    ...
    public List<SeckillActivityDO> getSeckillActivityBySpuIdsAndStatusAndDateTimeLt(Collection<Long> spuIds,
            Integer status, LocalDateTime dateTime) {
        // 1.查询出指定 spuId 的 spu 参加的活动最接近现在的一条记录。多个的话，一个 spuId 对应一个最近的活动编号
        List<Map<String, Object>> spuIdAndActivityIdMaps = seckillActivityMapper
                .selectSpuIdAndActivityIdMapsBySpuIdsAndStatus(spuIds, status);
        ...
        return seckillActivityMapper.selectListByIdsAndDateTimeLt(
                ...
                CollectionUtils.convertSet(spuIdAndActivityIdMaps, map -> MapUtil.getLong(map, "activityId")),
                dateTime);
