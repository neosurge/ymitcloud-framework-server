...
import com.ymitcloud.module.promotion.dal.mysql.coupon.CouponMapper;
...
public class CouponServiceImpl implements CouponService {
    ...
    public PageResult<CouponDO> getCouponPage(CouponPageReqVO pageReqVO) {
        ...
        if (StrUtil.isNotEmpty(pageReqVO.getNickname())) {
            ...
            pageReqVO.setUserIds(CollectionUtils.convertSet(users, MemberUserRespDTO::getId));
    ...
    public void useCoupon(Long id, Long userId, Long orderId) {
        ...
        int updateCount = couponMapper.updateByIdAndStatus(id, CouponStatusEnum.UNUSED.getStatus(), new CouponDO()
                .setStatus(CouponStatusEnum.USED.getStatus()).setUseOrderId(orderId).setUseTime(LocalDateTime.now()));
    ...
    public void returnUsedCoupon(Long id) {
        ...
        Integer status = LocalDateTimeUtils.beforeNow(coupon.getValidEndTime()) ? CouponStatusEnum.EXPIRE.getStatus() // 退还时可能已经过期了
    ...
    public void takeCoupon(Long templateId, Set<Long> userIds, CouponTakeTypeEnum takeType) {
        ...
        couponMapper.insertBatch(
                CollectionUtils.convertList(userIds, userId -> CouponConvert.INSTANCE.convert(template, userId)));
    ...
    public void takeCouponByRegister(Long userId) {
        ...
        List<CouponTemplateDO> templates = couponTemplateService
                .getCouponTemplateListByTakeType(CouponTakeTypeEnum.REGISTER);
    ...
    public List<CouponDO> getMatchCouponList(Long userId, AppCouponMatchReqVO matchReqVO) {
        return couponMapper.selectListByUserIdAndStatusAndUsePriceLeAndProductScope(userId,
                ...
                CouponStatusEnum.UNUSED.getStatus(), matchReqVO.getPrice(), matchReqVO.getSpuIds(),
                matchReqVO.getCategoryIds());
    ...
    public int expireCoupon() {
        ...
        List<CouponDO> list = couponMapper.selectListByStatusAndValidEndTimeLe(CouponStatusEnum.UNUSED.getStatus(),
                LocalDateTime.now());
    ...
    public Map<Long, Boolean> getUserCanCanTakeMap(Long userId, List<CouponTemplateDO> templates) {
        ...
        Map<Long, Boolean> userCanTakeMap = CollectionUtils.convertMap(templates, CouponTemplateDO::getId,
                templateId -> true);
        ...
        Set<Long> templateIds = CollectionUtils.convertSet(templates, CouponTemplateDO::getId,
                template -> template.getTakeLimitCount() != -1);
    ...
    private void validateCouponTemplateCanTake(CouponTemplateDO couponTemplate, Set<Long> userIds,
            CouponTakeTypeEnum takeType) {
    ...
    private void removeTakeLimitUser(Set<Long> userIds, CouponTemplateDO couponTemplate) {
        ...
        List<CouponDO> alreadyTakeCoupons = couponMapper.selectListByTemplateIdAndUserId(couponTemplate.getId(),
                userIds);
        ...
        Map<Long, Integer> userTakeCountMap = CollStreamUtil.groupBy(alreadyTakeCoupons, CouponDO::getUserId,
