...
import static com.ymitcloud.module.trade.enums.ErrorCodeConstants.PICK_UP_STORE_NOT_EXISTS;
import static com.ymitcloud.module.trade.enums.ErrorCodeConstants.PRICE_CALCULATE_DELIVERY_PRICE_TEMPLATE_NOT_FOUND;
...
import com.ymitcloud.module.trade.enums.delivery.DeliveryExpressChargeModeEnum;
...
import com.ymitcloud.module.trade.service.price.bo.TradePriceCalculateRespBO.OrderItem;
...
public class TradeDeliveryPriceCalculator implements TradePriceCalculator {
    ...
    private void calculateExpress(TradePriceCalculateReqBO param, TradePriceCalculateRespBO result) {
        ...
        List<OrderItem> selectedItem = CollectionUtils.filterList(result.getItems(), OrderItem::getSelected);
        Set<Long> deliveryTemplateIds = CollectionUtils.convertSet(selectedItem, OrderItem::getDeliveryTemplateId);
        Map<Long, DeliveryExpressTemplateRespBO> expressTemplateMap = deliveryExpressTemplateService
                .getExpressTemplateMapByIdsAndArea(deliveryTemplateIds, address.getAreaId());
    ...
    private boolean isGlobalExpressFree(TradePriceCalculateRespBO result) {
        ...
        return config != null && Boolean.TRUE.equals(config.getDeliveryExpressFreeEnabled()) // 开启包邮
    ...
    private void calculateDeliveryPrice(List<OrderItem> selectedSkus,
            ...
            Map<Long, DeliveryExpressTemplateRespBO> expressTemplateMap, TradePriceCalculateRespBO result) {
        // 按商品运费模板来计算商品的运费：相同的运费模板可能对应多条订单商品 SKU
        Map<Long, List<OrderItem>> template2ItemMap = CollectionUtils.convertMultiMap(selectedSkus,
                OrderItem::getDeliveryTemplateId);
        // 依次计算快递运费
