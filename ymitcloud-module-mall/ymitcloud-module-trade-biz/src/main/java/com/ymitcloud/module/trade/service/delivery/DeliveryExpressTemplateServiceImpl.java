package com.ymitcloud.module.trade.service.delivery;
...
import com.ymitcloud.module.trade.controller.admin.delivery.vo.expresstemplate.*;

import com.ymitcloud.module.trade.dal.dataobject.delivery.DeliveryExpressTemplateChargeDO;
import com.ymitcloud.module.trade.dal.dataobject.delivery.DeliveryExpressTemplateDO;
import com.ymitcloud.module.trade.dal.dataobject.delivery.DeliveryExpressTemplateFreeDO;
import com.ymitcloud.module.trade.dal.mysql.delivery.DeliveryExpressTemplateChargeMapper;
import com.ymitcloud.module.trade.dal.mysql.delivery.DeliveryExpressTemplateFreeMapper;
import com.ymitcloud.module.trade.dal.mysql.delivery.DeliveryExpressTemplateMapper;
import com.ymitcloud.module.trade.service.delivery.bo.DeliveryExpressTemplateRespBO;
import com.ymitcloud.module.trade.controller.admin.delivery.vo.expresstemplate.*;
import com.ymitcloud.module.trade.convert.delivery.DeliveryExpressTemplateConvert;
import com.ymitcloud.module.trade.enums.ErrorCodeConstants;
...
public class DeliveryExpressTemplateServiceImpl implements DeliveryExpressTemplateService {
    ...
    public Long createDeliveryExpressTemplate(DeliveryExpressTemplateCreateReqVO createReqVO) {
        ...
        if (CollUtil.isNotEmpty(createReqVO.getCharges())) {
            ...
            expressTemplateChargeMapper.insertBatch(DeliveryExpressTemplateConvert.INSTANCE.convertTemplateChargeList(
                    template.getId(), createReqVO.getChargeMode(), createReqVO.getCharges()));
        }
        // 插入运费模板包邮表
        if (CollUtil.isNotEmpty(createReqVO.getFrees())) {
            expressTemplateFreeMapper.insertBatch(DeliveryExpressTemplateConvert.INSTANCE
                    .convertTemplateFreeList(template.getId(), createReqVO.getFrees()));
    ...
    private void updateExpressTemplateFree(Long templateId, List<DeliveryExpressTemplateFreeBaseVO> frees) {
        ...
        List<DeliveryExpressTemplateFreeDO> newList = DeliveryExpressTemplateConvert.INSTANCE
                .convertTemplateFreeList(templateId, frees);
        ...
        if (CollUtil.isNotEmpty(diffList.get(2))) {
            ...
            expressTemplateFreeMapper
                    .deleteBatchIds(CollectionUtils.convertList(diffList.get(2), DeliveryExpressTemplateFreeDO::getId));
        }
    }

    private void updateExpressTemplateCharge(Long templateId, Integer chargeMode,
            List<DeliveryExpressTemplateChargeBaseVO> charges) {
        // 第一步，对比新老数据，获得添加、修改、删除的列表
        List<DeliveryExpressTemplateChargeDO> oldList = expressTemplateChargeMapper.selectListByTemplateId(templateId);
        List<DeliveryExpressTemplateChargeDO> newList = DeliveryExpressTemplateConvert.INSTANCE
                .convertTemplateChargeList(templateId, chargeMode, charges);
        List<List<DeliveryExpressTemplateChargeDO>> diffList = CollectionUtils.diffList(oldList, newList,
                (oldVal, newVal) -> {
        ...
        if (CollUtil.isNotEmpty(diffList.get(2))) {
            ...
            expressTemplateChargeMapper.deleteBatchIds(
                    CollectionUtils.convertList(diffList.get(2), DeliveryExpressTemplateChargeDO::getId));
    ...
    public PageResult<DeliveryExpressTemplateDO> getDeliveryExpressTemplatePage(
            DeliveryExpressTemplatePageReqVO pageReqVO) {
    ...
    public Map<Long, DeliveryExpressTemplateRespBO> getExpressTemplateMapByIdsAndArea(Collection<Long> ids,
            Integer areaId) {
