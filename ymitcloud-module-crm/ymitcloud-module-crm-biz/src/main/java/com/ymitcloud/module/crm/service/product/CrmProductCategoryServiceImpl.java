package com.ymitcloud.module.crm.service.product;
...
import com.ymitcloud.module.crm.dal.mysql.product.CrmProductCategoryMapper;
...
public class CrmProductCategoryServiceImpl implements CrmProductCategoryService {
    ...
    private void validateParentProductCategory(Long id) {
        // 如果是根分类，无需验证
        if (Objects.equals(id, CrmProductCategoryDO.PARENT_ID_NULL)) {
            return;
        }
        // 父分类不存在
        ...
        // 父分类不能是二级分类
        if (!Objects.equals(category.getParentId(), CrmProductCategoryDO.PARENT_ID_NULL)) {
