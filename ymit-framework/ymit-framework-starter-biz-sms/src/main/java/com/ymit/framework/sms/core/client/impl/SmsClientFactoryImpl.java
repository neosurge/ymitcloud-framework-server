...
import com.ymit.framework.sms.core.client.impl.aliyun.AliyunSmsClient;
import com.ymit.framework.sms.core.client.impl.debug.DebugDingTalkSmsClient;
import com.ymit.framework.sms.core.client.impl.tencent.TencentSmsClient;
...
public class SmsClientFactoryImpl implements SmsClientFactory {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SmsClientFactoryImpl.class);
    /**
     * 短信客户端 Map
    ...
    public SmsClientFactoryImpl() {
        ...
        Arrays.stream(SmsChannelEnum.values()).forEach(channel -> {
            ...
            SmsChannelProperties properties = new SmsChannelProperties().setCode(channel.getCode()).setApiKey("default default").setApiSecret("default");
            // 创建 Sms 客户端
            AbstractSmsClient smsClient = this.createSmsClient(properties);
            this.channelCodeClients.put(channel.getCode(), smsClient);
        });
    }

    @Override
    public SmsClient getSmsClient(Long channelId) {
        return this.channelIdClients.get(channelId);
    }

    @Override
    public SmsClient getSmsClient(String channelCode) {
        return this.channelCodeClients.get(channelCode);
    }

    @Override
    public void createOrUpdateSmsClient(SmsChannelProperties properties) {
        AbstractSmsClient client = this.channelIdClients.get(properties.getId());
        if (client == null) {
            client = this.createSmsClient(properties);
            client.init();
            this.channelIdClients.put(client.getId(), client);
        } else {
