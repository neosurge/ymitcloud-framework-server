package com.ymit.framework.redis.core;
...
public class TimeoutRedisCacheManager extends RedisCacheManager {
    ...
    protected RedisCache createRedisCache(@NonNull String name, RedisCacheConfiguration cacheConfig) {
        ...
        if (cacheConfig != null) {
            ...
            // 解析时间
            Duration duration = this.parseDuration(names[1]);
    ...
    private Duration parseDuration(String ttlStr) {
        ...
        switch (timeUnit) {
            case "d":
                return Duration.ofDays(this.removeDurationSuffix(ttlStr));
            case "h":
                return Duration.ofHours(this.removeDurationSuffix(ttlStr));
            case "m":
                return Duration.ofMinutes(this.removeDurationSuffix(ttlStr));
            case "s":
                return Duration.ofSeconds(this.removeDurationSuffix(ttlStr));
            default:
