...
public class YmitWebSecurityConfigurerAdapter {
    ...
    public YmitWebSecurityConfigurerAdapter(WebProperties webProperties, SecurityProperties securityProperties, AuthenticationEntryPoint authenticationEntryPoint, AccessDeniedHandler accessDeniedHandler, TokenAuthenticationFilter authenticationTokenFilter, List<AuthorizeRequestsCustomizer> authorizeRequestsCustomizers, ApplicationContext applicationContext) {
    ...
    protected SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity
                ...
                .exceptionHandling(c -> c.authenticationEntryPoint(this.authenticationEntryPoint).accessDeniedHandler(this.accessDeniedHandler));
        ...
        httpSecurity
                ...
                .authorizeHttpRequests(c -> c
                        ...
                        // 1.2 基于 ymit.security.permit-all-urls 无需认证
                        .requestMatchers(this.securityProperties.getPermitAllUrls().toArray(new String[0])).permitAll()
                        // 1.3 设置 App API 无需认证
                        .requestMatchers(this.buildAppApi("/**")).permitAll())
                // ②：每个项目的自定义规则
                .authorizeHttpRequests(c -> this.authorizeRequestsCustomizers.forEach(customizer -> customizer.customize(c)))
                // ③：兜底规则，必须认证
