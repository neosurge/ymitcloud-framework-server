package com.ymit.framework.web.core.handler;

import cn.hutool.core.exceptions.ExceptionUtil;
import cn.hutool.core.map.MapUtil;
import cn.hutool.core.util.StrUtil;
import com.ymit.framework.apilog.core.service.ApiErrorLog;
...
import static com.ymit.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    public GlobalExceptionHandler(ApiErrorLogFrameworkService apiErrorLogFrameworkService, Environment env) {
    ...
    public CommonResult<?> allExceptionHandler(HttpServletRequest request, Throwable ex) {
        ...
        if (ex instanceof ConstraintViolationException) {
            return this.constraintViolationExceptionHandler((ConstraintViolationException) ex);
        }
        if (ex instanceof ValidationException) {
            return this.validationException((ValidationException) ex);
        }
        if (ex instanceof NoHandlerFoundException) {
            return this.noHandlerFoundExceptionHandler(request, (NoHandlerFoundException) ex);
        }
        ...
        if (ex instanceof AccessDeniedException) {
            return this.accessDeniedExceptionHandler(request, (AccessDeniedException) ex);
        }
    ...
    public CommonResult<?> defaultExceptionHandler(HttpServletRequest req, Throwable ex) {
        // 情况一：处理表不存在的异常
        CommonResult<?> tableNotExistsResult = this.handleTableNotExists(ex);
        if (tableNotExistsResult != null) {
            return tableNotExistsResult;
        }
        // 情况二：部分特殊的库的处理
        if (Objects.equals("io.github.resilience4j.ratelimiter.RequestNotPermitted", ex.getClass().getName())) {
            return this.requestNotPermittedExceptionHandler(req, ex);
        }
        // 情况三：处理异常
        ...
        if (Arrays.asList(this.env.getActiveProfiles()).contains("local")) {
    ...
    private void createExceptionLog(HttpServletRequest req, Throwable e) {
        ...
        try {
            // 初始化 errorLog
            this.initExceptionLog(errorLog, req, e);
            // 执行插入 errorLog
    ...
    private void initExceptionLog(ApiErrorLog errorLog, HttpServletRequest request, Throwable e) {
        ...
        Map<String, Object> requestParams = MapUtil.<String, Object>builder().put("query", ServletUtils.getParamMap(request)).put("body", ServletUtils.getBody(request)).build();
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[报表模块 ymit-module-report - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[报表模块 ymit-module-report - 表结构未导入]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块 ymit-module-bpm - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[工作流模块 ymit-module-bpm - 表结构未导入]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号 ymit-module-mp - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[微信公众号 ymit-module-mp - 表结构未导入]");
        }
        // 4. 商城系统
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统 ymit-module-mall - 已禁用]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[商城系统 ymit-module-mall - 已禁用]");
        }
        // 5. 支付平台
        if (message.contains("pay_")) {
            log.error("[支付模块 ymit-module-pay - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(), "[支付模块 ymit-module-pay - 表结构未导入]");
        }
        return null;
    }
}