package com.ymit.framework.tenant.core.security;
...
public class TenantSecurityWebFilter extends ApiRequestFilter {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(TenantSecurityWebFilter.class);
    ...
    public TenantSecurityWebFilter(TenantProperties tenantProperties, WebProperties webProperties,
                                   GlobalExceptionHandler globalExceptionHandler, TenantFrameworkService tenantFrameworkService) {
        super(webProperties);
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
        ...
        if (user != null) {
            ...
            if (tenantId == null) {
                ...
                log.error("[doFilterInternal][租户({}) User({}/{}) 越权访问租户({}) URL({}/{})]", user.getTenantId(), user.getId(), user.getUserType(), TenantContextHolder.getTenantId(), request.getRequestURI(), request.getMethod());
    ...
    private boolean isIgnoreUrl(HttpServletRequest request) {
        // 快速匹配，保证性能
        if (CollUtil.contains(this.tenantProperties.getIgnoreUrls(), request.getRequestURI())) {
            return true;
        }
        // 逐个 Ant 路径匹配
        for (String url : this.tenantProperties.getIgnoreUrls()) {
