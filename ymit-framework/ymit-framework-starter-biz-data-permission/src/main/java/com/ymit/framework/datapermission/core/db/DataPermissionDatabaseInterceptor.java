package com.ymit.framework.datapermission.core.db;
...
import com.ymit.framework.common.util.collection.SetUtils;
...
public class DataPermissionDatabaseInterceptor extends JsqlParserSupport implements InnerInterceptor {
    ...
    MappedStatementCache getMappedStatementCache() {
    ...
    public void beforeQuery(Executor executor, MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) {
        ...
        List<DataPermissionRule> rules = this.ruleFactory.getDataPermissionRule(ms.getId());
        if (this.mappedStatementCache.noRewritable(ms, rules)) { // 如果无需重写，则跳过
            return;
        }
    ...
    protected Expression builderExpression(Expression currentExpression, List<Table> tables) {
        ...
        for (Table table : tables) {
            ...
            Expression expression = this.buildDataPermissionExpression(table);
            if (expression == null) {
                continue;
            }
    ...
    static final class MappedStatementCache {
        ...
        public void addNoRewritable(MappedStatement ms, List<DataPermissionRule> rules) {
            for (DataPermissionRule rule : rules) {
                ...
                if (CollUtil.isNotEmpty(mappedStatementIds)) {
                    ...
                    this.noRewritableMappedStatements.put(rule.getClass(), SetUtils.asSet(ms.getId()));
                }
            }
        }
         ...
         */
        public void clear() {
            this.noRewritableMappedStatements.clear();
        }
    }
}
