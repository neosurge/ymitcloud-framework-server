...
public abstract class AbstractRedisStreamMessageListener<T extends AbstractRedisStreamMessage> implements StreamListener<String, ObjectRecord<String, String>> {
    ...
    protected AbstractRedisStreamMessageListener() throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        this.messageType = this.getMessageClass();
        this.streamKey = this.messageType.getDeclaredConstructor().newInstance().getStreamKey();
    }
    ...
    public void onMessage(ObjectRecord<String, String> message) {
        // 消费消息
        T messageObj = JsonUtils.parseObject(message.getValue(), this.messageType);
        try {
            this.consumeMessageBefore(messageObj);
            // 消费消息
            this.onMessage(messageObj);
            // ack 消息消费完成
            this.redisMQTemplate.getRedisTemplate().opsForStream().acknowledge(this.group, message);
            // 1. 处理异常的情况
            // 2. 发送日志；以及事务的结合
            // 3. 消费日志；以及通用的幂等性
