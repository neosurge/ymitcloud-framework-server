package com.ymit.framework.quartz.core.scheduler;
...
import com.ymit.framework.quartz.core.handler.JobHandlerInvoker;
import org.quartz.*;

/**
 * {@link org.quartz.Scheduler} 的管理器，负责创建任务
 * <p>
 * 考虑到实现的简洁性，我们使用 jobHandlerName 作为唯一标识，即： 1. Job 的 {@link JobDetail#getKey()}
...
public class SchedulerManager {
    ...
    public void addJob(Long jobId, String jobHandlerName, String jobHandlerParam, String cronExpression, Integer retryCount, Integer retryInterval) throws SchedulerException {
        ...
        // 创建 JobDetail 对象
        JobDetail jobDetail = JobBuilder.newJob(JobHandlerInvoker.class).usingJobData(JobDataKeyEnum.JOB_ID.name(), jobId).usingJobData(JobDataKeyEnum.JOB_HANDLER_NAME.name(), jobHandlerName).withIdentity(jobHandlerName).build();
        // 创建 Trigger 对象
    ...
    public void updateJob(String jobHandlerName, String jobHandlerParam, String cronExpression, Integer retryCount, Integer retryInterval) throws SchedulerException {
        ...
        // 修改调度
        this.scheduler.rescheduleJob(new TriggerKey(jobHandlerName), newTrigger);
    }
    ...
    public void deleteJob(String jobHandlerName) throws SchedulerException {
        this.validateScheduler();
        this.scheduler.deleteJob(new JobKey(jobHandlerName));
    }
    ...
    public void pauseJob(String jobHandlerName) throws SchedulerException {
        this.validateScheduler();
        this.scheduler.pauseJob(new JobKey(jobHandlerName));
    }
    ...
    public void resumeJob(String jobHandlerName) throws SchedulerException {
        this.validateScheduler();
        this.scheduler.resumeJob(new JobKey(jobHandlerName));
        this.scheduler.resumeTrigger(new TriggerKey(jobHandlerName));
    }
    ...
    public void triggerJob(Long jobId, String jobHandlerName, String jobHandlerParam) throws SchedulerException {
        ...
        this.scheduler.triggerJob(new JobKey(jobHandlerName), data);
    }
