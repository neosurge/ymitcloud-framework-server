package com.ymit.framework.websocket.core.session;
...
public class WebSocketSessionManagerImpl implements WebSocketSessionManager {
    ...
    public void addSession(WebSocketSession session) {
        ...
        ConcurrentMap<Long, CopyOnWriteArrayList<WebSocketSession>> userSessionsMap = this.userSessions.get(user.getUserType());
        if (userSessionsMap == null) {
            userSessionsMap = new ConcurrentHashMap<>();
            if (this.userSessions.putIfAbsent(user.getUserType(), userSessionsMap) != null) {
                userSessionsMap = this.userSessions.get(user.getUserType());
            }
        }
    ...
    public WebSocketSession getSession(String id) {
        return this.idSessions.get(id);
    }

    @Override
    public Collection<WebSocketSession> getSessionList(Integer userType) {
        ConcurrentMap<Long, CopyOnWriteArrayList<WebSocketSession>> userSessionsMap = this.userSessions.get(userType);
