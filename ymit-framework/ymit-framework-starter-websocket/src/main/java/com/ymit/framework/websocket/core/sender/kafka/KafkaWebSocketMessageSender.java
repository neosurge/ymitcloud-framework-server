...
public class KafkaWebSocketMessageSender extends AbstractWebSocketMessageSender {
    private final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(KafkaWebSocketMessageSender.class);
    ...
    public void send(Integer userType, Long userId, String messageType, String messageContent) {
        this.sendKafkaMessage(null, userId, userType, messageType, messageContent);
    }

    @Override
    public void send(Integer userType, String messageType, String messageContent) {
        this.sendKafkaMessage(null, null, userType, messageType, messageContent);
    }

    @Override
    public void send(String sessionId, String messageType, String messageContent) {
        this.sendKafkaMessage(sessionId, null, null, messageType, messageContent);
    }
    ...
    private void sendKafkaMessage(String sessionId, Long userId, Integer userType, String messageType, String messageContent) {
        ...
        try {
            this.kafkaTemplate.send(this.topic, mqMessage).get();
        } catch (InterruptedException | ExecutionException e) {
            this.log.error("[sendKafkaMessage][发送消息({}) 到 Kafka 失败]", mqMessage, e);
        }
    }
}
