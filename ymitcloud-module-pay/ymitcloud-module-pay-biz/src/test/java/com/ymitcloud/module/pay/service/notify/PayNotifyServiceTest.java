package com.ymitcloud.module.pay.service.notify;
...
import com.ymitcloud.module.pay.dal.mysql.notify.PayNotifyLogMapper;
import com.ymitcloud.module.pay.dal.mysql.notify.PayNotifyTaskMapper;
import com.ymitcloud.module.pay.dal.redis.notify.PayNotifyLockRedisDAO;
import com.ymitcloud.module.pay.enums.notify.PayNotifyStatusEnum;
import com.ymitcloud.module.pay.enums.notify.PayNotifyTypeEnum;
import com.ymitcloud.module.pay.framework.job.config.PayJobConfiguration;
...
import com.ymitcloud.module.pay.service.refund.PayRefundServiceImpl;
...
import static com.ymitcloud.framework.test.core.util.RandomUtils.randomString;
...
public class PayNotifyServiceTest extends BaseDbUnitTest {
    ...
    public void testCreatePayNotifyTask_order() {
        ...
        try (MockedStatic<SpringUtil> springUtilMockedStatic = mockStatic(SpringUtil.class)) {
            ...
            assertThat(dbTask).extracting("type", "dataId", "status", "notifyTimes", "maxNotifyTimes", "appId",
                    "merchantOrderId", "notifyUrl").containsExactly(type, dataId,
                            PayNotifyStatusEnum.WAITING.getStatus(), 0, 9, order.getAppId(), order.getMerchantOrderId(),
                            order.getNotifyUrl());
    ...
    public void testCreatePayNotifyTask_refund() {
        ...
        try (MockedStatic<SpringUtil> springUtilMockedStatic = mockStatic(SpringUtil.class)) {
            ...
            assertThat(dbTask).extracting("type", "dataId", "status", "notifyTimes", "maxNotifyTimes", "appId",
                    "merchantOrderId", "notifyUrl").containsExactly(type, dataId,
                            PayNotifyStatusEnum.WAITING.getStatus(), 0, 9, refund.getAppId(),
    ...
    public void testExecuteNotify() throws InterruptedException {
        ...
        PayNotifyTaskDO dbTask01 = randomPojo(PayNotifyTaskDO.class, o -> o
                .setStatus(PayNotifyStatusEnum.WAITING.getStatus()).setNextNotifyTime(addTime(Duration.ofMinutes(-1))));
    ...
    public void testExecuteNotify0_exception() {
        ...
        PayNotifyTaskDO task = randomPojo(PayNotifyTaskDO.class,
                o -> o.setType(-1).setNotifyTimes(0).setMaxNotifyTimes(9));
    ...
    public void testProcessNotifyResult_success() {
        ...
        PayNotifyTaskDO task = randomPojo(PayNotifyTaskDO.class, o -> o.setNotifyTimes(0).setMaxNotifyTimes(9));
    ...
    public void testProcessNotifyResult_failure() {
        ...
        PayNotifyTaskDO task = randomPojo(PayNotifyTaskDO.class, o -> o.setNotifyTimes(8).setMaxNotifyTimes(9));
    ...
    private void mockLock(Long id) {
        ...
        if (id == null) {
            ...
            when(redissonClient.getLock(eq("pay_notify:lock:" + id))).thenReturn(lock);
