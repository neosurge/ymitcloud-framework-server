package com.ymitcloud.module.pay.service.transfer;
...
import com.ymitcloud.framework.pay.core.client.dto.transfer.PayTransferRespDTO;
import com.ymitcloud.framework.pay.core.client.dto.transfer.PayTransferUnifiedReqDTO;
import com.ymitcloud.framework.pay.core.enums.transfer.PayTransferStatusRespEnum;
...
import com.ymitcloud.module.pay.dal.mysql.transfer.PayTransferMapper;
import com.ymitcloud.module.pay.dal.redis.no.PayNoRedisDAO;
import com.ymitcloud.module.pay.enums.notify.PayNotifyTypeEnum;
import com.ymitcloud.module.pay.enums.transfer.PayTransferStatusEnum;
...
public class PayTransferServiceImpl implements PayTransferService {
    ...
    private void notifyTransferInProgress(PayChannelDO channel, PayTransferRespDTO notify) {
        ...
        if (PayTransferStatusEnum.isInProgress(transfer.getStatus())) { // 如果已经是转账中，直接返回，不用重复更新
            return;
        }
        if (!PayTransferStatusEnum.isWaiting(transfer.getStatus())) {
        ...
        int updateCounts = transferMapper.updateByIdAndStatus(transfer.getId(),
                CollUtil.newArrayList(PayTransferStatusEnum.WAITING.getStatus()),
                new PayTransferDO().setStatus(PayTransferStatusEnum.IN_PROGRESS.getStatus()));
        if (updateCounts == 0) {
            throw exception(PAY_TRANSFER_STATUS_IS_NOT_WAITING);
        }
    ...
    private void notifyTransferClosed(PayChannelDO channel, PayTransferRespDTO notify) {
        ...
        int updateCount = transferMapper.updateByIdAndStatus(transfer.getId(),
                CollUtil.newArrayList(PayTransferStatusEnum.WAITING.getStatus(), PayTransferStatusEnum.IN_PROGRESS.getStatus()),
                new PayTransferDO().setStatus(PayTransferStatusEnum.CLOSED.getStatus()).setChannelId(channel.getId())
    ...
    public int syncTransfer() {
        List<PayTransferDO> list = transferMapper.selectListByStatus(PayTransferStatusEnum.WAITING.getStatus());
