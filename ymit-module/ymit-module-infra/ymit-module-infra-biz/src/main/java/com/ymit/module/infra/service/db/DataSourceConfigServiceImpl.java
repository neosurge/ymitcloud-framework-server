package com.ymit.module.infra.service.db;

import com.baomidou.dynamic.datasource.creator.DataSourceProperty;
import com.baomidou.dynamic.datasource.spring.boot.autoconfigure.DynamicDataSourceProperties;
import com.ymit.framework.common.util.object.BeanUtils;
import com.ymit.framework.mybatis.core.util.JdbcUtils;
...
import com.ymit.module.infra.dal.mysql.db.DataSourceConfigMapper;
...
import static com.ymit.module.infra.enums.ErrorCodeConstants.DATA_SOURCE_CONFIG_NOT_EXISTS;
import static com.ymit.module.infra.enums.ErrorCodeConstants.DATA_SOURCE_CONFIG_NOT_OK;
...
public class DataSourceConfigServiceImpl implements DataSourceConfigService {
    ...
    public Long createDataSourceConfig(DataSourceConfigSaveReqVO createReqVO) {
        ...
        this.validateConnectionOK(config);

        // 插入
        this.dataSourceConfigMapper.insert(config);
        // 返回
        return config.getId();
    }
    ...
    public void updateDataSourceConfig(DataSourceConfigSaveReqVO updateReqVO) {
        // 校验存在
        this.validateDataSourceConfigExists(updateReqVO.getId());
        DataSourceConfigDO updateObj = BeanUtils.toBean(updateReqVO, DataSourceConfigDO.class);
        this.validateConnectionOK(updateObj);

        // 更新
        this.dataSourceConfigMapper.updateById(updateObj);
    }

    @Override
    public void deleteDataSourceConfig(Long id) {
        // 校验存在
        this.validateDataSourceConfigExists(id);
        // 删除
        this.dataSourceConfigMapper.deleteById(id);
    }

    private void validateDataSourceConfigExists(Long id) {
        if (this.dataSourceConfigMapper.selectById(id) == null) {
    ...
    public DataSourceConfigDO getDataSourceConfig(Long id) {
        ...
        if (Objects.equals(id, DataSourceConfigDO.ID_MASTER)) {
            return this.buildMasterDataSourceConfig();
        }
        // 从 DB 中读取
        return this.dataSourceConfigMapper.selectById(id);
    }

    @Override
    public List<DataSourceConfigDO> getDataSourceConfigList() {
        List<DataSourceConfigDO> result = this.dataSourceConfigMapper.selectList();
        // 补充 master 数据源
        result.add(0, this.buildMasterDataSourceConfig());
        return result;
    }
    ...
    private DataSourceConfigDO buildMasterDataSourceConfig() {
        String primary = this.dynamicDataSourceProperties.getPrimary();
        DataSourceProperty dataSourceProperty = this.dynamicDataSourceProperties.getDatasource().get(primary);
