package com.ymit.module.infra.service.codegen;
...
import com.ymit.module.infra.dal.mysql.codegen.CodegenColumnMapper;
import com.ymit.module.infra.dal.mysql.codegen.CodegenTableMapper;
...
import com.ymit.module.infra.service.codegen.inner.CodegenBuilder;
import com.ymit.module.infra.service.codegen.inner.CodegenEngine;
import com.ymit.module.infra.service.db.DatabaseTableService;
import com.ymit.module.system.api.user.AdminUserApi;
...
import static com.ymit.framework.common.util.collection.CollectionUtils.convertMap;
...
public class CodegenServiceImpl implements CodegenService {
    ...
    public List<Long> createCodegenList(Long userId, CodegenCreateListReqVO reqVO) {
        ...
        reqVO.getTableNames().forEach(tableName -> ids.add(this.createCodegen(userId, reqVO.getDataSourceConfigId(), tableName)));
        return ids;
    }

    private Long createCodegen(Long userId, Long dataSourceConfigId, String tableName) {
        // 从数据库中，获得数据库表结构
        TableInfo tableInfo = this.databaseTableService.getTable(dataSourceConfigId, tableName);
        // 导入
        return this.createCodegen0(userId, dataSourceConfigId, tableInfo);
    }

    private Long createCodegen0(Long userId, Long dataSourceConfigId, TableInfo tableInfo) {
        // 校验导入的表和字段非空
        this.validateTableInfo(tableInfo);
        // 校验是否已经存在
        if (this.codegenTableMapper.selectByTableNameAndDataSourceConfigId(tableInfo.getName(),
                dataSourceConfigId) != null) {
            throw exception(CODEGEN_TABLE_EXISTS);
        }

        // 构建 CodegenTableDO 对象，插入到 DB 中
        CodegenTableDO table = this.codegenBuilder.buildTable(tableInfo);
        table.setDataSourceConfigId(dataSourceConfigId);
        table.setScene(CodegenSceneEnum.ADMIN.getScene()); // 默认配置下，使用管理后台的模板
        table.setFrontType(this.codegenProperties.getFrontType());
        table.setAuthor(this.userApi.getUser(userId).getNickname());
        this.codegenTableMapper.insert(table);

        // 构建 CodegenColumnDO 数组，插入到 DB 中
        List<CodegenColumnDO> columns = this.codegenBuilder.buildColumns(table.getId(), tableInfo.getFields());
        for (var item : columns) {
            if (StringUtils.isEmpty(item.getDictType())) {
                item.setDictType("");
            }
        }
        // 如果没有主键，则使用第一个字段作为主键
        ...
        this.codegenColumnMapper.insertBatch(columns);
        return table.getId();
    }
    ...
    public void updateCodegen(CodegenUpdateReqVO updateReqVO) {
        // 校验是否已经存在
        if (this.codegenTableMapper.selectById(updateReqVO.getTable().getId()) == null) {
            throw exception(CODEGEN_TABLE_NOT_EXISTS);
        }
        // 校验主表字段存在
        if (Objects.equals(updateReqVO.getTable().getTemplateType(), CodegenTemplateTypeEnum.SUB.getType())) {
            if (this.codegenTableMapper.selectById(updateReqVO.getTable().getMasterTableId()) == null) {
        ...
        this.codegenTableMapper.updateById(updateTableObj);
        // 更新 column 字段定义
        List<CodegenColumnDO> updateColumnObjs = BeanUtils.toBean(updateReqVO.getColumns(), CodegenColumnDO.class);
        updateColumnObjs.forEach(updateColumnObj -> this.codegenColumnMapper.updateById(updateColumnObj));
    }
    ...
    public void syncCodegenFromDB(Long tableId) {
        // 校验是否已经存在
        CodegenTableDO table = this.codegenTableMapper.selectById(tableId);
        if (table == null) {
            throw exception(CODEGEN_TABLE_NOT_EXISTS);
        }
        // 从数据库中，获得数据库表结构
        TableInfo tableInfo = this.databaseTableService.getTable(table.getDataSourceConfigId(), table.getTableName());
        // 执行同步
        this.syncCodegen0(tableId, tableInfo);
    }
    ...
    private void syncCodegen0(Long tableId, TableInfo tableInfo) {
        ...
        List<CodegenColumnDO> codegenColumns = this.codegenColumnMapper.selectListByTableId(tableId);
        ...
        // 4.1 插入新增的字段
        List<CodegenColumnDO> columns = this.codegenBuilder.buildColumns(tableId, tableFields);
        this.codegenColumnMapper.insertBatch(columns);
        // 4.2 删除不存在的字段
        if (CollUtil.isNotEmpty(deleteColumnIds)) {
            this.codegenColumnMapper.deleteBatchIds(deleteColumnIds);
        }
    }
    ...
    public void deleteCodegen(Long tableId) {
        // 校验是否已经存在
        if (this.codegenTableMapper.selectById(tableId) == null) {
            throw exception(CODEGEN_TABLE_NOT_EXISTS);
        }

        // 删除 table 表定义
        this.codegenTableMapper.deleteById(tableId);
        // 删除 column 字段定义
        this.codegenColumnMapper.deleteListByTableId(tableId);
    }

    @Override
    public List<CodegenTableDO> getCodegenTableList(Long dataSourceConfigId) {
        return this.codegenTableMapper.selectListByDataSourceConfigId(dataSourceConfigId);
    }

    @Override
    public PageResult<CodegenTableDO> getCodegenTablePage(CodegenTablePageReqVO pageReqVO) {
        return this.codegenTableMapper.selectPage(pageReqVO);
    }

    @Override
    public CodegenTableDO getCodegenTable(Long id) {
        return this.codegenTableMapper.selectById(id);
    }

    @Override
    public List<CodegenColumnDO> getCodegenColumnListByTableId(Long tableId) {
        return this.codegenColumnMapper.selectListByTableId(tableId);
    }
    ...
    public Map<String, String> generationCodes(Long tableId) {
        ...
        List<CodegenColumnDO> columns = this.codegenColumnMapper.selectListByTableId(tableId);
        ...
        if (CodegenTemplateTypeEnum.isMaster(table.getTemplateType())) {
            // 校验子表存在
            subTables = this.codegenTableMapper.selectListByTemplateTypeAndMasterTableId(
            ...
            for (CodegenTableDO subTable : subTables) {
                List<CodegenColumnDO> subColumns = this.codegenColumnMapper.selectListByTableId(subTable.getId());
        ...
        // 执行生成
        return this.codegenEngine.execute(table, columns, subTables, subColumnsList);
    }

    @Override
    public List<DatabaseTableRespVO> getDatabaseTableList(Long dataSourceConfigId, String name, String comment) {
        List<TableInfo> tables = this.databaseTableService.getTableList(dataSourceConfigId, name, comment);
        // 移除在 Codegen 中，已经存在的
        Set<String> existsTables = convertSet(
                this.codegenTableMapper.selectListByDataSourceConfigId(dataSourceConfigId), CodegenTableDO::getTableName);
