...
public class CodegenTableSaveReqVO {
    ...
    public boolean isParentMenuIdValid() {
        ...
        return ObjectUtil.notEqual(this.getScene(), CodegenSceneEnum.ADMIN.getScene()) || this.getParentMenuId() != null;
    }

    @AssertTrue(message = "关联的父表信息不全")
    @JsonIgnore
    public boolean isSubValid() {
        return ObjectUtil.notEqual(this.getTemplateType(), CodegenTemplateTypeEnum.SUB)
                || (ObjectUtil.isAllNotEmpty(this.masterTableId, this.subJoinColumnId, this.subJoinMany));
    }

    @AssertTrue(message = "关联的树表信息不全")
    @JsonIgnore
    public boolean isTreeValid() {
        return ObjectUtil.notEqual(this.templateType, CodegenTemplateTypeEnum.TREE)
                || (ObjectUtil.isAllNotEmpty(this.treeParentColumnId, this.treeNameColumnId));
    }
