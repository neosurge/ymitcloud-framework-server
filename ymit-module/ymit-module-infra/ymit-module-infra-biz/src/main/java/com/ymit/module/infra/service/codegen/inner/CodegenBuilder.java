...
import com.ymit.module.infra.enums.codegen.CodegenColumnHtmlTypeEnum;
import com.ymit.module.infra.enums.codegen.CodegenColumnListConditionEnum;
...
public class CodegenBuilder {
    ...
    private static final Map<String, CodegenColumnHtmlTypeEnum> COLUMN_HTML_TYPE_MAPPINGS =
            MapUtil.<String, CodegenColumnHtmlTypeEnum>builder()
                    ...
                    .put("gender", CodegenColumnHtmlTypeEnum.RADIO)
    ...
    public List<CodegenColumnDO> buildColumns(Long tableId, List<TableField> tableFields) {
        ...
        for (CodegenColumnDO column : columns) {
            ...
            // 初始化 Column 列的默认字段
            this.processColumnOperation(column); // 处理 CRUD 相关的字段的默认值
            this.processColumnUI(column); // 处理 UI 相关的字段的默认值
    ...
    private void processColumnExample(CodegenColumnDO column) {
        ...
        if (StrUtil.endWithIgnoreCase(column.getJavaField(), "name")) {
            column.setExample(randomEle(new String[]{"张三", "李四", "王五", "赵六", "云码"}));
            return;
        }
        // status
        ...
        if (StrUtil.endWithIgnoreCase(column.getColumnName(), "url")) {
            column.setExample("https://www.ymit.com");
            return;
        }
        // reason
