...
import com.ymit.module.system.dal.mysql.social.SocialClientMapper;
    ...
    private final LoadingCache<String, WxMpService> wxMpServiceCache = CacheUtils.buildAsyncReloadingCache(
            ...
            new CacheLoader<String, WxMpService>() {
                ...
                public WxMpService load(String key) {
                    ...
                    return SocialClientServiceImpl.this.buildWxMpService(keys[0], keys[1]);
                }
    ...
    private final LoadingCache<String, WxMaService> wxMaServiceCache = CacheUtils.buildAsyncReloadingCache(
            ...
            new CacheLoader<String, WxMaService>() {
                ...
                public WxMaService load(String key) {
                    String[] keys = key.split(":");
                    return SocialClientServiceImpl.this.buildWxMaService(keys[0], keys[1]);
                }
...
//        AuthRequest authRequest = this.buildAuthRequest(socialType, userType);
//        // 生成跳转地址
...
//        AuthRequest request = this.authRequestFactory.get(SocialTypeEnum.valueOfType(socialType).getSource());
//        Assert.notNull(request, String.format("社交平台(%d) 不存在", socialType));
//        // 2. 查询 DB 的配置项，如果存在则进行覆盖
//        SocialClientDO client = this.socialClientMapper.selectBySocialTypeAndUserType(socialType, userType);
...
//    }
    ...
    public WxJsapiSignature createWxMpJsapiSignature(Integer userType, String url) {
        WxMpService service = this.getWxMpService(userType);
    ...
    WxMpService getWxMpService(Integer userType) {
        ...
        SocialClientDO client = this.socialClientMapper.selectBySocialTypeAndUserType(
                SocialTypeEnum.WECHAT_MP.getType(), userType);
        if (client != null && Objects.equals(client.getStatus(), CommonStatusEnum.ENABLE.getStatus())) {
            return this.wxMpServiceCache.getUnchecked(client.getClientId() + ":" + client.getClientSecret());
        }
    ...
    public WxMpService buildWxMpService(String clientId, String clientSecret) {
        ...
        WxMpRedisConfigImpl configStorage = new WxMpRedisConfigImpl(
                new RedisTemplateWxRedisOps(this.stringRedisTemplate),
                this.wxMpProperties.getConfigStorage().getKeyPrefix());
    ...
    public WxMaPhoneNumberInfo getWxMaPhoneNumberInfo(Integer userType, String phoneCode) {
        WxMaService service = this.getWxMaService(userType);
        try {
    ...
    WxMaService getWxMaService(Integer userType) {
        ...
        if (client != null && Objects.equals(client.getStatus(), CommonStatusEnum.ENABLE.getStatus())) {
            return this.wxMaServiceCache.getUnchecked(client.getClientId() + ":" + client.getClientSecret());
        }
    ...
    private WxMaService buildWxMaService(String clientId, String clientSecret) {
        ...
        WxMaRedisBetterConfigImpl configStorage = new WxMaRedisBetterConfigImpl(
                new RedisTemplateWxRedisOps(this.stringRedisTemplate),
                this.wxMaProperties.getConfigStorage().getKeyPrefix());
        configStorage.setAppid(clientId);
        configStorage.setSecret(clientSecret);
    ...
    public Long createSocialClient(SocialClientSaveReqVO createReqVO) {
        // 校验重复
        this.validateSocialClientUnique(null, createReqVO.getUserType(), createReqVO.getSocialType());

        // 插入
        SocialClientDO client = BeanUtils.toBean(createReqVO, SocialClientDO.class);
        this.socialClientMapper.insert(client);
        return client.getId();
    }
    ...
    public void updateSocialClient(SocialClientSaveReqVO updateReqVO) {
        // 校验存在
        this.validateSocialClientExists(updateReqVO.getId());
        // 校验重复
        this.validateSocialClientUnique(updateReqVO.getId(), updateReqVO.getUserType(), updateReqVO.getSocialType());

        // 更新
        SocialClientDO updateObj = BeanUtils.toBean(updateReqVO, SocialClientDO.class);
        this.socialClientMapper.updateById(updateObj);
    }

    @Override
    public void deleteSocialClient(Long id) {
        // 校验存在
        this.validateSocialClientExists(id);
        // 删除
        this.socialClientMapper.deleteById(id);
    }

    private void validateSocialClientExists(Long id) {
        if (this.socialClientMapper.selectById(id) == null) {
     ...
     * <p>
     * 原因是，不同端（userType）选择某个社交登录（socialType）时，需要通过 #buildAuthRequest(Integer, Integer)构建对应的请求
     *
     * @param id         编号
     * @param userType   用户类型
     * @param socialType 社交类型
    ...
    public SocialClientDO getSocialClient(Long id) {
        return this.socialClientMapper.selectById(id);
    }

    @Override
    public PageResult<SocialClientDO> getSocialClientPage(SocialClientPageReqVO pageReqVO) {
        return this.socialClientMapper.selectPage(pageReqVO);
    }
