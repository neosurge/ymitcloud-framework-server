...
import com.ymit.module.system.dal.dataobject.social.SocialUserBindDO;
import com.ymit.module.system.dal.dataobject.social.SocialUserDO;
import com.ymit.module.system.dal.mysql.social.SocialUserBindMapper;
import com.ymit.module.system.dal.mysql.social.SocialUserMapper;
...
public class SocialUserServiceImpl implements SocialUserService {
    ...
    public List<SocialUserDO> getSocialUserList(Long userId, Integer userType) {
        // 获得绑定
        List<SocialUserBindDO> socialUserBinds = this.socialUserBindMapper.selectListByUserIdAndUserType(userId, userType);
        if (CollUtil.isEmpty(socialUserBinds)) {
            return Collections.emptyList();
        }
        // 获得社交用户
        return this.socialUserMapper.selectBatchIds(convertSet(socialUserBinds, SocialUserBindDO::getSocialUserId));
    }
    ...
    public String bindSocialUser(SocialUserBindReqDTO reqDTO) {
        // 获得社交用户
        SocialUserDO socialUser = this.authSocialUser(reqDTO.getSocialType(), reqDTO.getUserType(),
                reqDTO.getCode(), reqDTO.getState());
        Assert.notNull(socialUser, "社交用户不能为空");

        // 社交用户可能之前绑定过别的用户，需要进行解绑
        this.socialUserBindMapper.deleteByUserTypeAndSocialUserId(reqDTO.getUserType(), socialUser.getId());

        // 用户可能之前已经绑定过该社交类型，需要进行解绑
        this.socialUserBindMapper.deleteByUserTypeAndUserIdAndSocialType(reqDTO.getUserType(), reqDTO.getUserId(),
                socialUser.getType());
        ...
        this.socialUserBindMapper.insert(socialUserBind);
    ...
    public void unbindSocialUser(Long userId, Integer userType, Integer socialType, String openid) {
        ...
        SocialUserDO socialUser = this.socialUserMapper.selectByTypeAndOpenid(socialType, openid);
        if (socialUser == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SOCIAL_USER_NOT_FOUND);
        }

        // 获得对应的社交绑定关系
        this.socialUserBindMapper.deleteByUserTypeAndUserIdAndSocialType(userType, userId, socialUser.getType());
    }

    @Override
    public SocialUserRespDTO getSocialUser(Integer userType, Integer socialType, String code, String state) {
        // 获得社交用户
        SocialUserDO socialUser = this.authSocialUser(socialType, userType, code, state);
        Assert.notNull(socialUser, "社交用户不能为空");

        // 如果未绑定的社交用户，则无法自动登录，进行报错
        SocialUserBindDO socialUserBind = this.socialUserBindMapper.selectByUserTypeAndSocialUserId(userType,
                socialUser.getId());
...
//        return socialUser;
    ...
    public SocialUserDO getSocialUser(Long id) {
        return this.socialUserMapper.selectById(id);
    }

    @Override
    public PageResult<SocialUserDO> getSocialUserPage(SocialUserPageReqVO pageReqVO) {
        return this.socialUserMapper.selectPage(pageReqVO);
    }
