...
import com.ymit.framework.common.util.http.HttpUtils;
...
import com.ymit.module.system.convert.oauth2.OAuth2OpenConvert;
...
import com.ymit.module.system.enums.oauth2.OAuth2GrantTypeEnum;
import com.ymit.module.system.service.oauth2.OAuth2ApproveService;
import com.ymit.module.system.service.oauth2.OAuth2ClientService;
import com.ymit.module.system.service.oauth2.OAuth2GrantService;
...
import static com.ymit.framework.common.exception.enums.GlobalErrorCodeConstants.BAD_REQUEST;
import static com.ymit.framework.common.exception.util.ServiceExceptionUtil.exception0;
...
public class OAuth2OpenController {
    ...
    public CommonResult<OAuth2OpenAccessTokenRespVO> postAccessToken(HttpServletRequest request, @RequestParam("grant_type") String grantType, @RequestParam(value = "code", required = false) String code, // 授权码模式
        ...
        // 1.2 校验客户端
        String[] clientIdAndSecret = this.obtainBasicAuthorization(request);
        OAuth2ClientDO client = this.oauth2ClientService.validOAuthClientFromCache(clientIdAndSecret[0], clientIdAndSecret[1], grantType, scopes, redirectUri);

        // 2. 根据授权模式，获取访问令牌
        OAuth2AccessTokenDO accessTokenDO;
        switch (grantTypeEnum) {
            case AUTHORIZATION_CODE:
                accessTokenDO = this.oauth2GrantService.grantAuthorizationCodeForAccessToken(client.getClientId(), code, redirectUri, state);
                break;
            case PASSWORD:
                accessTokenDO = this.oauth2GrantService.grantPassword(username, password, client.getClientId(), scopes);
                break;
            case CLIENT_CREDENTIALS:
                accessTokenDO = this.oauth2GrantService.grantClientCredentials(client.getClientId(), scopes);
                break;
            case REFRESH_TOKEN:
                accessTokenDO = this.oauth2GrantService.grantRefreshToken(refreshToken, client.getClientId());
                break;
            default:
                throw new IllegalArgumentException("未知授权类型：" + grantType);
        }
        Assert.notNull(accessTokenDO, "访问令牌不能为空"); // 防御性检查
        return success(OAuth2OpenConvert.convert(accessTokenDO));
    }
    ...
    public CommonResult<Boolean> revokeToken(HttpServletRequest request, @RequestParam("token") String token) {
        // 校验客户端
        String[] clientIdAndSecret = this.obtainBasicAuthorization(request);
        OAuth2ClientDO client = this.oauth2ClientService.validOAuthClientFromCache(clientIdAndSecret[0], clientIdAndSecret[1], null, null, null);

        // 删除访问令牌
        return success(this.oauth2GrantService.revokeToken(client.getClientId(), token));
    }
    ...
    public CommonResult<OAuth2OpenCheckTokenRespVO> checkToken(HttpServletRequest request, @RequestParam("token") String token) {
        // 校验客户端
        String[] clientIdAndSecret = this.obtainBasicAuthorization(request);
        this.oauth2ClientService.validOAuthClientFromCache(clientIdAndSecret[0], clientIdAndSecret[1], null, null, null);

        // 校验令牌
        OAuth2AccessTokenDO accessTokenDO = this.oauth2TokenService.checkAccessToken(token);
        Assert.notNull(accessTokenDO, "访问令牌不能为空"); // 防御性检查
        return success(OAuth2OpenConvert.convert2(accessTokenDO));
    }
    ...
    public CommonResult<OAuth2OpenAuthorizeInfoRespVO> authorize(@RequestParam("clientId") String clientId) {
        ...
        // 1. 获得 Client 客户端的信息
        OAuth2ClientDO client = this.oauth2ClientService.validOAuthClientFromCache(clientId);
        // 2. 获得用户已经授权的信息
        List<OAuth2ApproveDO> approves = this.oauth2ApproveService.getApproveList(getLoginUserId(), this.getUserType(), clientId);
        // 拼接返回
        return success(OAuth2OpenConvert.convert(client, approves));
    }
    ...
    public CommonResult<String> approveOrDeny(@RequestParam("response_type") String responseType, @RequestParam("client_id") String clientId, @RequestParam(value = "scope", required = false) String scope, @RequestParam("redirect_uri") String redirectUri, @RequestParam(value = "auto_approve") Boolean autoApprove, @RequestParam(value = "state", required = false) String state) {
        @SuppressWarnings("unchecked") Map<String, Boolean> scopes = JsonUtils.parseObject(scope, Map.class);
        ...
        OAuth2ClientDO client = this.oauth2ClientService.validOAuthClientFromCache(clientId, null, grantTypeEnum.getGrantType(), scopes.keySet(), redirectUri);

        // 2.1 假设 approved 为 null，说明是场景一
        if (Boolean.TRUE.equals(autoApprove)) {
            // 如果无法自动授权通过，则返回空 url，前端不进行跳转
            if (!this.oauth2ApproveService.checkForPreApproval(getLoginUserId(), this.getUserType(), clientId, scopes.keySet())) {
                return success(null);
            }
        } else { // 2.2 假设 approved 非 null，说明是场景二
            // 如果计算后不通过，则跳转一个错误链接
            if (!this.oauth2ApproveService.updateAfterApproval(getLoginUserId(), this.getUserType(), clientId, scopes)) {
        ...
        if (grantTypeEnum == OAuth2GrantTypeEnum.AUTHORIZATION_CODE) {
            return success(this.getAuthorizationCodeRedirect(getLoginUserId(), client, approveScopes, redirectUri, state));
        }
        // 3.2 如果是 token 则是 implicit 简化模式，则发送 accessToken 访问令牌，并重定向
        return success(this.getImplicitGrantRedirect(getLoginUserId(), client, approveScopes, redirectUri, state));
    }

    private String getImplicitGrantRedirect(Long userId, OAuth2ClientDO client, List<String> scopes, String redirectUri, String state) {
    ...
    private String getAuthorizationCodeRedirect(Long userId, OAuth2ClientDO client, List<String> scopes, String redirectUri, String state) {
        // 1. 创建 code 授权码
        String authorizationCode = this.oauth2GrantService.grantAuthorizationCodeForCode(userId, this.getUserType(), client.getClientId(), scopes, redirectUri, state);
        // 2. 拼接重定向的 URL
