package com.ymit.module.system.service.errorcode;
...
import com.ymit.module.system.dal.mysql.errorcode.ErrorCodeMapper;
...
public class ErrorCodeServiceImpl implements ErrorCodeService {
    ...
    public Long createErrorCode(ErrorCodeSaveReqVO createReqVO) {
        ...
        // 插入
        ErrorCodeDO errorCode = BeanUtils.toBean(createReqVO, ErrorCodeDO.class).setType(ErrorCodeTypeEnum.MANUAL_OPERATION.getType());
    ...
    public void updateErrorCode(ErrorCodeSaveReqVO updateReqVO) {
        ...
        // 更新
        ErrorCodeDO updateObj = BeanUtils.toBean(updateReqVO, ErrorCodeDO.class).setType(ErrorCodeTypeEnum.MANUAL_OPERATION.getType());
        errorCodeMapper.updateById(updateObj);
    }

    @Override
    public void deleteErrorCode(Collection<?> ids) {
        // 校验存在
        if (CollectionUtils.isEmpty(ids)) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.ERROR_CODE_NOT_EXISTS);
        }
        Long count = errorCodeMapper.selectCount(new LambdaQueryWrapper<ErrorCodeDO>().in(ErrorCodeDO::getId, ids));
        if (count.compareTo(ids.stream().count()) != 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.ERROR_CODE_NOT_EXISTS);
        }
        // 删除
        errorCodeMapper.deleteBatchIds(ids);
    }
    ...
    public void autoGenerateErrorCodes(List<ErrorCodeAutoGenerateReqDTO> autoGenerateDTOs) {
        ...
        // 获得错误码
        List<ErrorCodeDO> errorCodeDOs = errorCodeMapper.selectListByCodes(convertSet(autoGenerateDTOs, ErrorCodeAutoGenerateReqDTO::getCode));
        ...
        autoGenerateDTOs.forEach(autoGenerateDTO -> {
            ...
            // 不存在，则进行新增
            if (errorCode == null) {
                errorCode = BeanUtils.toBean(autoGenerateDTO, ErrorCodeDO.class).setType(ErrorCodeTypeEnum.AUTO_GENERATION.getType());
                errorCodeMapper.insert(errorCode);
                return;
            }
            // 存在，则进行更新。更新有三个前置条件：
            ...
            if (!autoGenerateDTO.getApplicationName().equals(errorCode.getApplicationName())) {
                log.error("[autoGenerateErrorCodes][自动创建({}/{}) 错误码失败，数据库中已经存在({}/{})]", autoGenerateDTO.getCode(), autoGenerateDTO.getApplicationName(), errorCode.getCode(), errorCode.getApplicationName());
                return;
            }
            // 条件 3. 错误提示语存在差异
    ...
    public List<ErrorCodeRespDTO> getErrorCodeList(String applicationName, LocalDateTime minUpdateTime) {
        List<ErrorCodeDO> list = errorCodeMapper.selectListByApplicationNameAndUpdateTimeGt(applicationName, minUpdateTime);
