package com.ymitcloud.framework.web.core.handler;

import static com.ymitcloud.framework.common.exception.enums.GlobalErrorCodeConstants.BAD_REQUEST;
import static com.ymitcloud.framework.common.exception.enums.GlobalErrorCodeConstants.FORBIDDEN;
import static com.ymitcloud.framework.common.exception.enums.GlobalErrorCodeConstants.INTERNAL_SERVER_ERROR;
import static com.ymitcloud.framework.common.exception.enums.GlobalErrorCodeConstants.METHOD_NOT_ALLOWED;
import static com.ymitcloud.framework.common.exception.enums.GlobalErrorCodeConstants.NOT_FOUND;
import static com.ymitcloud.framework.common.exception.enums.GlobalErrorCodeConstants.NOT_IMPLEMENTED;
import static com.ymitcloud.framework.common.exception.enums.GlobalErrorCodeConstants.TOO_MANY_REQUESTS;
...
import com.ymitcloud.framework.apilog.core.service.ApiErrorLog;
...
public class GlobalExceptionHandler {
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[报表模块 ymitcloud-module-report - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[报表模块 ymitcloud-module-report - 表结构未导入]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块 ymitcloud-module-bpm - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[工作流模块 ymitcloud-module-bpm - 表结构未导入]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号 ymitcloud-module-mp - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[微信公众号 ymitcloud-module-mp - 表结构未导入]");
        }
        // 4. 商城系统
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统 ymitcloud-module-mall - 已禁用]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[商城系统 ymitcloud-module-mall - 已禁用]");
        }
        // 5. 支付平台
        if (message.contains("pay_")) {
            log.error("[支付模块 ymitcloud-module-pay - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[支付模块 ymitcloud-module-pay - 表结构未导入]");
        }
        return null;
    }
