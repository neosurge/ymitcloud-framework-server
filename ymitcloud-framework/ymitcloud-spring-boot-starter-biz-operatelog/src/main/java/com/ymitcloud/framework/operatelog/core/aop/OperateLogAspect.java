package com.ymitcloud.framework.operatelog.core.aop;

import static com.ymitcloud.framework.common.exception.enums.GlobalErrorCodeConstants.INTERNAL_SERVER_ERROR;
import static com.ymitcloud.framework.common.exception.enums.GlobalErrorCodeConstants.SUCCESS;
...
import com.ymitcloud.framework.common.util.monitor.TracerUtils;
import com.ymitcloud.framework.common.util.servlet.ServletUtils;
import com.ymitcloud.framework.operatelog.core.enums.OperateTypeEnum;
import com.ymitcloud.framework.operatelog.core.service.OperateLog;
...
/**
 * 拦截使用 @OperateLog 注解，如果满足条件，则生成操作日志。 满足如下任一条件，则会进行记录： 1. 使用 @ApiOperation +
 * 非 @GetMapping 2. 使用 @OperateLog 注解
 * <p>
...
public class OperateLogAspect {
    ...
    @Around("@annotation(com.ymitcloud.framework.operatelog.core.annotations.OperateLog) && @annotation(operateLog)")
    public Object around(ProceedingJoinPoint joinPoint,
            com.ymitcloud.framework.operatelog.core.annotations.OperateLog operateLog) throws Throwable {
    ...
    private void log(ProceedingJoinPoint joinPoint,
            com.ymitcloud.framework.operatelog.core.annotations.OperateLog operateLog, LocalDateTime startTime,
        ...
        try {
            ...
            log.error("[log][记录操作日志时，发生异常，其中参数是 joinPoint({}) operateLog({}) result({}) exception({}) ]", joinPoint,
                    operateLog, result, exception, ex);
        }
    }
    ...
    private static void fillModuleFields(OperateLog operateLogObj, ProceedingJoinPoint joinPoint,
            com.ymitcloud.framework.operatelog.core.annotations.OperateLog operateLog) {
        // module 属性
        if (operateLog != null) {
    ...
    private static void fillMethodFields(OperateLog operateLogObj, ProceedingJoinPoint joinPoint,
            com.ymitcloud.framework.operatelog.core.annotations.OperateLog operateLog, LocalDateTime startTime,
            Object result, Throwable exception) {
    ...
    private static RequestMethod obtainFirstLogRequestMethod(RequestMethod[] requestMethods) {
        ...
        return Arrays
                .stream(requestMethods).filter(requestMethod -> requestMethod == RequestMethod.POST
                        || requestMethod == RequestMethod.PUT || requestMethod == RequestMethod.DELETE)
    ...
    private static RequestMethod obtainFirstMatchRequestMethod(RequestMethod[] requestMethods) {
        ...
        // 然后，匹配次优的 GET
        result = Arrays.stream(requestMethods).filter(requestMethod -> requestMethod == RequestMethod.GET).findFirst()
                .orElse(null);
        if (result != null) {
            return result;
        }
